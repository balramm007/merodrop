      ...
      {isAppLoading ? (
        ...
        >
          ...
          >
            ...
            {/* Route-Isolated Content */}
            <Routes>
              {/* HOME ROUTE: Full sharing UI */}
              <Route path="/" element={
                <div className="relative w-full h-full flex flex-col min-h-screen overflow-hidden z-10">
                  {/* Sonar Radar — anchored to footer logo */}
                  <Radar theme={settings.theme} />

                  <div className="flex-1 flex flex-col items-center justify-center pb-40">
                    {/* Tagline: slides up when peers found */}
                    <motion.div
                      layout
                      initial={{ y: 0, scale: 1 }}
                      animate={{
                      ...
                      className="flex flex-col items-center z-20 relative max-w-4xl px-4 gpu-accelerated"
                      style={{ willChange: 'transform' }}
                    >
                      <h1 className={`text-4xl md:text-6xl font-bold mb-4 text-center tracking-tight leading-tight transition-colors duration-300 ${
                        settings.theme === 'light' ? 'text-[#000000]' : 'text-white'
                      }`}>
                        {peerList.length > 0 ? 'Open MeroDrop to share files easily!' : 'Waiting for users...'}
                      </h1>
                      <p className={`text-lg md:text-2xl font-medium max-w-md text-center ${
                              ...
                              <AnimatePresence mode="popLayout">
                                {peerList.map((peer) => (
                                  <PeerAvatar
                                    ...
                                    onContextMenu={(_e, p) => setChatTarget(p)}
                  ...
                  <div className="fixed bottom-0 inset-x-0 flex flex-col items-center justify-end z-50 pointer-events-none pb-8">
                    {/* Logo + Ripple Rings */}
                    <div className="relative mb-4 pointer-events-auto group" style={{ willChange: 'transform' }}>
                      <div className="absolute inset-0 bg-blue-500/25 rounded-full animate-ripple" style={{ animationDelay: '0s', willChange: 'transform, opacity' }} />
                      <div className="absolute inset-0 bg-blue-500/25 rounded-full animate-ripple" style={{ animationDelay: '1s', willChange: 'transform, opacity' }} />
                      <div className="absolute inset-0 bg-blue-500/25 rounded-full animate-ripple" style={{ animationDelay: '2s', willChange: 'transform, opacity' }} />
                      <div className="absolute inset-0 bg-blue-500/25 rounded-full animate-ripple" style={{ animationDelay: '3s', willChange: 'transform, opacity' }} />
                      <div className="relative w-16 h-16 bg-black/50 backdrop-blur-xl rounded-full border border-white/10 flex items-center justify-center shadow-2xl z-10 group-hover:scale-110 transition-transform duration-300 gpu-accelerated animate-logo-beat overflow-hidden">
                        <img src={LOGO_URL} alt="Logo" className="w-14 h-14 rounded-full object-cover" />
                      </div>
                    </div>

                    {/* Identity Row — pill is ALWAYS center, links float right on desktop */}
                    <div className="relative w-full flex flex-col items-center gap-3 md:gap-0 pointer-events-auto">
                      {/* Perfectly centered Identity Pill */}
                      <motion.div
                        className={`flex items-center cursor-pointer group px-6 py-3 rounded-full border backdrop-blur-md transition-all gpu-accelerated shadow-2xl ${
                          settings.theme === 'light'
                            ? 'bg-[#000000] border-white/10 hover:bg-black/90'
                            : 'bg-[#fffefa] border-black/10 hover:bg-[#fffefa]/90'
                        }`}
                        style={{ willChange: 'transform' }}
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <span className={`text-[13px] font-bold ${
                            settings.theme === 'light' ? 'text-white' : 'text-black'
                          }`}>You are known as:</span>
                          <div className={`flex items-center px-3 py-1.5 rounded-full transition-all border ${
                            settings.theme === 'light'
                            ...
                            <Pencil size={12} className={`ml-2 ${
                              settings.theme === 'light' ? 'text-white/50' : 'text-black/40'
                            }`} />
                          </div>
                        </div>
                      </motion.div>

                      {/* Nav Links — absolute right on desktop, below on mobile */}
                      <div className={`
                        flex items-center gap-6 text-[13px] font-semibold
                      ...
                      `}>
                        {[
                          ...
                          <Link
                            key={to}
                            to={to}
                            className={`transition-all duration-200 opacity-40 hover:opacity-100 ${
                              settings.theme === 'light'
                                ? 'text-black'
                                : 'text-white'
                            }`}
                  ...
                  >
                    ...
                    <div className="flex w-full gap-4">
                      <button
                        ...
                        className="flex-1 py-4 bg-[#2c2c2e] text-[#9aa0a6] font-bold rounded-full transition-all duration-200 hover:bg-[#3a3a3c] active:scale-95"
                      >
                        Cancel
                      </button>
                      <button
