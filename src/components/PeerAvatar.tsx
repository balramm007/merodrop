...
const PeerAvatar: React.FC<PeerAvatarProps> = ({ peer, onClick, onContextMenu, progress = 0, isCompact = false }) => {
  ...
  }, [progress, isCompact, circumference]);
  ...
  return (
    <motion.div
      layout
      className="flex flex-col items-center justify-center cursor-pointer group pointer-events-auto select-none touch-none gpu-accelerated"
      ...
      <div className={`relative ${isCompact ? 'mb-3' : 'mb-6'} flex items-center justify-center`} style={{ width: size, height: size }}>
        <svg viewBox={`0 0 ${size} ${size}`} className="absolute inset-0 w-full h-full transform -rotate-90 drop-shadow-[0_0_15px_rgba(59,130,246,0.3)]">
          <circle
            ...
            fill="transparent"
            className="stroke-black/[0.1] dark:stroke-white/[0.05]"
          />
          <circle
            ref={ringRef}
            cx={center}
        ...
        {/* Core Icon Container */}
        <div className="relative w-[75%] h-[75%] rounded-full bg-white dark:bg-[#1c1c1e] flex items-center justify-center shadow-2xl border border-black/5 dark:border-white/5 group-hover:border-primary/50 transition-all duration-300">
          <Icon className="w-[45%] h-[45%] text-gray-500 dark:text-[#e8eaed] group-hover:text-primary transition-colors" strokeWidth={1.5} />
        </div>
        
        {/* Status dot */}
        <div className={`absolute bottom-[5%] right-[5%] ${isCompact ? 'w-4 h-4 border-2' : 'w-[25%] h-[25%] max-w-[24px] max-h-[24px] border-[3px]'} bg-green-500 border-white dark:border-black rounded-full shadow-[0_0_15px_rgba(34,197,94,0.7)] z-10`} />
      </div>

      {/* Name Label - Responsive */}
      <div className={`${isCompact ? 'px-3 py-1.5' : 'px-4 py-2'} bg-white dark:bg-[#202124] rounded-full border border-black/5 dark:border-white/5 shadow-xl group-hover:bg-gray-50 dark:group-hover:bg-[#2d2e31] transition-all text-center min-w-[80px] max-w-[160px]`}>
        <span className={`${isCompact ? 'text-[11px]' : 'text-[13px] md:text-[15px]'} font-bold text-gray-800 dark:text-white truncate block leading-snug tracking-tight max-w-[100px] mx-auto transition-colors duration-300`}>
          {peer.name}
        </span>
      </div>
    </motion.div>
